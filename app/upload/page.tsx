...
export default function UploadPage() {
  ...
  async function createZip(fileList: FileList): Promise<Blob> {
    ...
    for (let i = 0; i < fileList.length; i++) {
      ...
      // file.webkitRelativePath contains the path within the folder
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          {status === "authenticated" && (
            <div className="flex items-center gap-3">
              <Link href="/" className="mt-4 inline-flex h-9 items-center justify-center rounded-full border border-black/10 bg-white/70 px-4 text-xs font-semibold text-zinc-700 shadow-sm transition hover:bg-white/90 dark:border-white/10 dark:bg-zinc-950/60 dark:text-zinc-300 dark:hover:bg-zinc-950/80 sm:mt-0">
                Home
              </Link>
              <button
                onClick={() => signOut()}
                className="mt-4 inline-flex h-9 items-center justify-center rounded-full border border-black/10 bg-white/70 px-4 text-xs font-semibold text-zinc-700 shadow-sm transition hover:bg-white/90 dark:border-white/10 dark:bg-zinc-950/60 dark:text-zinc-300 dark:hover:bg-zinc-950/80 sm:mt-0"
              >
                Sign out
              </button>
            </div>
